      ...
      <table className='min-w-full bg-white border rounded-lg'>
        <thead>
          <tr>
            ...
            <th className='py-3 px-4 border-b text-left max=sm:hidden'>Data</th>
          ...
          {userApplications.map((job,index) => true ? (
            <tr key={index}>
              ...
              <td className='py-2 px-4 border-b'>
                <span className={`px-4 py-1.5 rounded  ${job.status === 'Accepted' ? 'bg-green-100' :job.status === 'Rejected' ? 'bg-red-100': 'bg-blue-100  '}`}>
                  {job.status}
                </span>
                </td>
            </tr>
