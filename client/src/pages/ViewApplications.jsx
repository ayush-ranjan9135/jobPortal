...
const ViewApplications = () => {
  ...
  const {backendUrl , compantToken} = useContext(AppContext)
  ...
  const changeJobApplicationStatus = async (id, status) => {
    try {
      const {data} =await axios.post(backendUrl+'/api/company/change-status',
        {id ,status},{headers:{token:compantToken}}
      )
      if(data.success){
              ...
              <tr key={index} className='text-gray-700'>
                ...
                <td className='py-2 px-4 border-b'>
                  <a href={applicant.userId.resume} target='_blank'>
                    ...
                    className='bg-blue-50 text-blue-400 py-3 py-1 rounded inline-flex gap-2 items-center'
                    />
                  </a>
                </td >
