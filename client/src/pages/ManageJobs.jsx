...
const ManageJobs = () => {
  ...
  //Function to fatch company job applications data
  ...
  try {
    const { data } = await axios.post(
      ...
      { id, visible }, // Send both ID and new visibility state
      { headers: { token: companyToken } }
    );
            ...
            {jobs.map((job,index) =>(
              <tr key={index} className='text-gray-700'>
                ...
                <td className='py-2 px-4 border-b'>
                  <input onChange={(e)=>changeJobVisibility(job._id, e.target.checked)}
